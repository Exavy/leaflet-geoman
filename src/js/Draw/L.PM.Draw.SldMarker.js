...
Draw.SldMarker = Draw.Marker.extend({
  initialize(map) {
    this._map = map;
    this._shape = 'SldMarker';
    this.toolbarButtonName = 'drawSldMarker';
  },
  enable(options) {
  ...
  isRelevantMarker(layer) {
    return layer instanceof L.SldMarker && layer.pm && !layer._pmTempLayer;
  },
  _createMarker(e) {
    if (!e.latlng) {
      return;
    ...
    // create marker
    const marker = L.sldMarker(latlng, this.options.pathOptions);
